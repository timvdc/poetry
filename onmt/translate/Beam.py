...
class Beam(object):
    ...
    def __init__(self, size, pad, bos, eos,
        ...
        self.rand_ys = [self.tt.LongTensor(size)
                        .fill_(pad)]
        self.rand_ys[0][0] = bos
    ...
    def advance(self, word_probs, attn_out):
        ...
        * `attn_out`- attention at the last step
        ...
        beam_scores = beam_scores.div(0.8)
        sm = self.s_func(beam_scores)
    ...
    def advance_prior(self, word_probs, attn_out):
        """
        Given prob over words for every last beam `wordLk` and attention
        `attn_out`: Compute and update the beam search.
        ...
        * `attn_out`- attention at the last step
        ...
        #print(beam_scores.numpy()[0], beam_scores.numpy()[0].sum(), )
        sm = self.s_func(beam_scores)
        prior = np.ones_like(beam_scores) * 1e-20
        prior[1733] = 0.95
        #print(prior)
        sm = sm.mul(torch.from_numpy(prior))
        sm = sm.div(sm.sum())
        sm = sm.log().div(0.4)
        sm = self.s_func(sm)
